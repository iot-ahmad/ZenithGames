...
import { GameStatus, IRBID_COLORS, ShopItem, RUN_SPEED_BASE } from '../../types';
...
export const HUD: React.FC = () => {
  ...
  const target = ['I', 'R', 'B', 'I', 'D'];

  useEffect(() => {
      if (status === GameStatus.VICTORY && !cosmicInsight && !loadingInsight) {
          setLoadingInsight(true);
          getCosmicInsight(score, level).then(insight => {
              setCosmicInsight(insight);
              setLoadingInsight(false);
          });
      }
      if (status !== GameStatus.VICTORY) {
          setCosmicInsight(null);
      }
  }, [status, score, level]);
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-purple-900/90 to-black/95 z-[100] text-white pointer-events-auto backdrop-blur-md overflow-y-auto">
            <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                ...
                <p className="text-cyan-300 text-sm md:text-2xl font-mono mb-8 tracking-widest text-center">
                    {loadingInsight ? "DECODING NEURAL SIGNALS..." : (cosmicInsight || "THE ANSWER TO THE UNIVERSE HAS BEEN FOUND")}
                </p>
